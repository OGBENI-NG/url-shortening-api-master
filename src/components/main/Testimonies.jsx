...
export default function Testimonies({testimoniesData}) {
  ...
  const renderBrandData = testimoniesData.map((data, index) => (
    ...
    >
      <div className={`relative bg-dark-violet -mt-[4.4rem] h-20 w-20 p-5 rounded-full mb-4
        ${index !== 0 ? 'before:absolute before:content-[""] before:w-3 before:h-20 before:bg-cyan before:left-[2.1rem] before:-top-[3.6rem] before:m-0 before:-z-[2]' : ''}`}>
  ...
  return (
    <Section className="bg-lightGray p-6 pt-12">
      {renderBrandData}
    </Section>
  );
